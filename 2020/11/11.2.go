...
func validcoord(floor [][]int8, row int, col int) bool{
	return (row >= 0 && row < len(floor) && col >= 0 && col < len(floor[0]))
}
...
func neighbour_count(floor [][]int8, row int, col int) int {
	...
	for _,n := range directions {
		n_row = row
		n_col = col
		for validcoord(floor, n_row + n[0], n_col + n[1]) {
			n_row += n[0]
			n_col += n[1]
			if(floor[n_row][n_col] == -1){
				break
			} else if(floor[n_row][n_col] == 1){
				ret++
				break
			}
		}
	...
	for changes > 0 {
		...
		for r := 0; r< rows; r++{
			for c:= 0; c< columns; c++{
				...
				if((neighbours == 0 && floor[r][c] == -1 )  || (neighbours >= 5 && floor[r][c] == 1)){
					changes++
